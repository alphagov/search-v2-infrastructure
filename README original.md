...
# Kickstart GCP - Vertex AI Search (Discovery Engine)
...
1. Clone the policy repo and copy contents of policy-library to new repo. Clone the repo at the same level of the `terraform-example-foundation` folder, the next instructions assume that layout.
   ...
   cd gcp-policies
   git checkout -b main
   cp -R ../kickstart-infrastructure-terraform-gcp-v1/policy-library/ .
   ```
...
1. Save `0-bootstrap` Terraform configuration to `gcp-bootstrap` source repository:
   ...
   cp -R ../kickstart-infrastructure-terraform-gcp-v1/0-bootstrap/ ./envs/shared
   cp ../kickstart-infrastructure-terraform-gcp-v1/build/cloudbuild-tf-* .
   cp ../kickstart-infrastructure-terraform-gcp-v1/build/tf-wrapper.sh .
