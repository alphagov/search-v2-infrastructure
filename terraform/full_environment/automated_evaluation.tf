...
resource "google_cloud_scheduler_job" "daily_search_evaluation" {
  ...
  http_target {
    ...
    body = base64encode(templatefile("${path.module}/files/automated_evaluation_default_datasets/config.tftpl",
      {
        gcs_urls       = [for file_jl in fileset("${path.module}/files/automated_evaluation_default_datasets/judgement_lists/", "*.csv") : { name = split(".csv", file_jl)[0], uri = join("", ["gcs://", google_storage_bucket_object.judgement_list, "/", file_jl]) }],
