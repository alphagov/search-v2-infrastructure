...
resource "google_cloud_scheduler_job" "daily_search_evaluation" {
  ...
  http_target {
    ...
    body        = base64encode(templatefile("./files/automated_evaluation_default_datasets/config.tftpl", { gcs_pub_with_parts_url =  concat("", ["gs://", google_storage_bucket_object.judgement_list_pub_with_parts.bucket , "/" ,google_storage_bucket_object.judgement_list_pub_with_parts.name]) }))
    headers = {
