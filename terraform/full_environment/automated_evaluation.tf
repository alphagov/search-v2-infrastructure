...
resource "google_cloud_scheduler_job" "daily_search_evaluation" {
  ...
  http_target {
    ...
    body = base64encode(templatefile("./files/automated_evaluation_default_datasets/config.tftpl",
      {
        gcs_urls       = [for file_jl in fileset("./terraform/full_environment/files/automated_evaluation_default_datasets/judgement_lists/", "*.csv") : { name = split(".csv", file_jl)[0], uri = join("", ["gcs://", google_storage_bucket_object.judgement_list_pub_with_parts.bucket, "/", file_jl]) }],
        gcs_output_url = join("", ["gcs://", google_storage_bucket.automated_evaluation_output.name])
    }))
    headers = {
